...
Names follow the same rule — readable words, no cryptic abbreviations.

| Purpose | Attribute | ✅ Correct | ❌ Wrong |
|---|---|---|---|
| DOM reference (cached in constructor) | `data-ref` | `data-ref="items-list"` | `data-ref="il"` |
| Action trigger | `data-action` | `data-action="remove-item"` | `data-action="ri"` |
| Open a modal | `data-action` | `data-action="modal-open-delete"` | `data-action="mod"` |
...
| JS-driven state | `data-state` | `data-state="active"` | — |

### 7.3 JS variables and properties
