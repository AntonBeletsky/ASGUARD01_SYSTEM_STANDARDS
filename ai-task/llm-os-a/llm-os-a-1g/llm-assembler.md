...
Two optional fields carry information `expect_hash` doesn't: `--context
'...'` echoes up to 120 characters of what's actually at this location,
for a reviewer scanning the batch's descriptors without dereferencing a
hash for each one; `--rule-ref '...'` points at the specific clause of
whatever governing material this finding answers to (a section number, an
anchor), for conformance-shaped tasks where "why is this even a
violation" should be answerable without re-deriving it. Neither is ever
checked against anything -- they're notes left for a human, not gates.

Building the map itself: pull every distinct region a source spans
before writing descriptors for it, not just the regions that turned out
to need a decision. A region with no open question still needs an entry
-- "transfer as-is" -- or it silently has no entry at all once the map is
read back later. See "Conflict-filtered inventory," an anti-pattern named
in the methodology document this skill implements.
...
For material that carries its own embedded logic -- a script, a handler,
anything that queries or assumes structure at runtime -- `domino-next`'s
search only finds cascades that leave a text trace. A dependency where
one part's code silently requires another part to still exist in a
specific place (a selector that returns nothing if a sibling moved) never
shows up as a match, because there's no recurring string to find. Judging
those means reading the dependent logic itself for what it assumes, not
just reviewing what the mechanical search handed back.
...
check will pass while still being wrong. Say what you checked it against
with `--checked-against path/to/file` -- `validate-plan` will name any
invariant missing one, on a batch that's actually carrying inserted
content, as a warning worth reading before you apply, not a rule it
enforces for you.
...
- **Global rename / terminology change** -- domino tracing *is* the
  ...
  twice, not once.
- **Content assembly / transplant** (build a new file by combining
  material drawn from two or more existing files, none of which is the
  reference standard) -- before writing a single descriptor, partition
  every source region you're pulling from into a complete range map:
  every line assigned "transfer as-is," "transfer with a resolved
  decision," or "deliberately excluded, stated why." Do this as its own
  pass over the raw region (`sed -n`, `wc -l`, whatever confirms the
  boundary), separate from the pass where you work out what needs a
  decision -- a list of open decisions is a filter to run over the
  inventory afterward, not the inventory itself. An `add-invariant` whose
  `--expected-count` is grepped from the same payload file a descriptor
  was just built from checks that the plan agrees with itself, not that
  it's complete; where you can, check a fact the payload doesn't
  determine instead (a markup attribute's count against a corresponding
  class definition's count, not both against the same source text).
...
**A reference the material points at that won't resolve here isn't
license to point it somewhere else instead.** A broken local path, a
missing asset, an external file this environment doesn't have -- carry
it forward exactly as written and say so, rather than quietly swapping
in something that renders. The mirror image of the anti-pattern above:
that one drops material nobody asked to have dropped; this one replaces
material nobody asked to have replaced. Both fail the same way -- not
loudly, and not where whoever asked for the work will notice without
being told.
