...
the name of this project: a six-phase task-execution methodology and a
tested implementation of it (`llm-assembler`, a skill for line-precise
batch editing of text and code).

This delivery contains **two independent, complete packagings of the same
underlying work**. They are not sequential — Set B doesn't supersede Set
A, Set A isn't a summary of Set B. They're the same content, organized
two different ways for two different uses. Pick whichever fits; nothing
here requires using both.

## Files at a glance

| File | Set | Lines | What it is |
|---|---|---|---|
| `llm-operational-platform-task-execution-methodology-full-in-one-file-md.md` | **A** | ~1085 | Methodology + implementation, merged into one document |
| `llm-operational-platform-task-execution-methodology.md` | **B** | ~350 | Methodology alone, implementation-agnostic |
| `llm-assembler.md` | **B** | ~445 | The tool's operating instructions, readable copy |
| `llm-assembler.skill` | **B** | — | The tool itself, packaged and installable |

## Set A — one merged, self-contained document

**`llm-operational-platform-task-execution-methodology-full-in-one-file-md.md`**
...
Phase 1.3's cascade tracing, Phase 3's execution ordering, and so on) is
...
**`llm-operational-platform-task-execution-methodology.md`**
...
`llm-assembler` is a specific tool's name, not the project's — it keeps
its own identity (the skill's `name:` field, its filenames) unchanged, the
same way a specific application keeps its own name on a platform. **LLM
Operational Platform** is the project both files above answer to: the
merged document in Set A opens with it, the standalone methodology
document in Set B opens with it, and `llm-assembler.md`'s own opening
line points back to both as the methodology it implements.
