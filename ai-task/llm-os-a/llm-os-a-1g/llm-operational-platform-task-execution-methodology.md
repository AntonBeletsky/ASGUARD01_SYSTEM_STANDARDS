...
## Task Execution Methodology — Abstract (Implementation-Agnostic)
...
the name of the project this document belongs to: a six-phase
task-execution methodology, and, separately, a tested implementation of
it (`llm-assembler`, a skill for line-precise batch editing of text and
code). This document covers the methodology alone.
...
**Three distinct activities, only one of which resists automation.** Tracing a single deviation's cascade decomposes into: *searching* the material for places that might be affected (a matter of coverage — did every relevant location get looked at), *judging* which of those places are genuine cascades and which are coincidental (a matter of understanding what the material means, not just what it contains), and *ordering* the resulting dependencies once they're known (a matter of respecting them correctly — never scheduling a dependent item before what it depends on, refusing to proceed if dependencies cycle). Searching and ordering are mechanical, verifiable operations: given a location to search from, there is a correct, exhaustive set of candidate matches; given a complete list of dependency edges, there is a correct order or a correct refusal. Judging is not mechanical in the same way — it requires understanding, and no amount of correctness in the search or the ordering compensates for a candidate that was wrongly accepted or wrongly dismissed. Treat these as three different reliability problems, checkable and improvable independently of one another — not one problem that either is or isn't "automated."

**This is its own pass, sequenced strictly after Phase 1.2 closes — never interleaved with it.** Cross-examination (1.2) produces a finished, stable list of deviations; domino analysis (1.3) then traces each one against that finished list. Running the two interleaved — tracing a deviation's cascade the moment it's found, before 1.2 has finished looking at everything else — makes deduplication unreliable (a cascade found early can't be checked against findings 1.2 hasn't produced yet) and makes progress through either pass ambiguous to measure or resume. This is the ANALYSIS→PLAN→EXECUTION rule from Core Philosophy applied one level down: cross-examination and cascade-tracing are different kinds of work, and merging them invites the same class of error merging the three top-level stages does.

**This is a durable, resumable pass, not a single sweep redone from scratch on interruption.** Each deviation from 1.2 becomes an item with its own traced-or-not status; tracing one is a unit of progress in exactly the sense applying one planned change is a unit of progress in Phase 4, and deserves the same treatment — recorded durably as it happens, checkable for whether it's already done, resumable from wherever it stopped. This matters more here than almost anywhere else in the methodology precisely because each trace is meant to be a genuine, unhurried look at the material: losing that work to an interruption and being forced to guess whether it needs redoing is a worse outcome than losing a cheaper unit of progress elsewhere.

**Nothing proceeds past Phase 1.3 with an untraced item.** The same "cheapest point to catch a mistake is before anything is touched" reasoning that governs Plan Validation in Phase 3 applies one phase earlier: a deviation that reaches the task map without having been traced is a silent gap in exactly the way an incomplete cascade produces a broken result. This is a gate, not an advisory step — a plan built from an incompletely-traced deviation list is invalid, the same as a plan with an unresolved dependency cycle.

**For material carrying its own embedded logic, judging requires reading that logic, not only searching text.** A dependency can exist with no shared wording at all — one part's logic silently assumes another part still exists in a specific place or still supplies a specific value, and nothing about that assumption appears as a matching string anywhere. Mechanical search, however exhaustive, produces no candidate for this kind of cascade, because there is nothing recurring to find. Judging it means reading the dependent logic itself for what it hard-codes or assumes — a container/child relationship a runtime query depends on, a value one part reads that another part must therefore keep supplying — in addition to, not instead of, searching for textual re-occurrence.
...
For conformance-shaped tasks, an issue may additionally carry a pointer to the specific clause of the governing reference responsible for it — distinct from Category, which ties it to this task's own taxonomy rather than to a location in the external document. Category groups; this pointer traces a single issue back to the exact place the requirement it violates actually comes from, useful when one category covers several distinct clauses with different wording or different exceptions, and when the reference document itself is later revised. This methodology does not parse or index governing reference material — Phase 0 loads it by ordinary reading — so such a pointer is necessarily an unverified citation, not a checkable reference the way a dependency is checkable against a real issue identifier.
...
3. **Final report.** A structured summary of status per unit, distinguishing fully clean units from units with remaining issues, broken down by severity. For every declared taxonomy category, the report records either at least one finding or an explicit statement that the category was checked and nothing was found — never silence; a category with nothing said about it is indistinguishable from a category nobody checked. Where the report cites a phase of this methodology as the basis for a decision, it states separately what that citation actually supplied — a process step, a required check, a structural rule — from what came from applying that step to the specific material at hand; a citation that blends the two reads as if the methodology itself supplied domain knowledge it does not contain.
...
**Content assembly / transplant** — building a new unit by combining material drawn from two or more existing units, none of which is itself the reference standard.
Partition every source region into a complete inventory before extraction — not only the regions a conflict or open decision was raised about; treat a region with no conflict as an inventory item requiring the disposition "transfer as-is," not as an item with no disposition at all.
...
- [ ] For every identified issue, has its cascade been traced as its own resumable pass — checkable per item, not a single attempt with no record of what's covered?
...
**Interleaved cross-examination and cascade-tracing**
Tracing a deviation's cascade the moment it's found, before cross-examination has finished looking at everything else, makes deduplication against not-yet-discovered findings unreliable and leaves progress through either pass impossible to measure or resume.
→ Correct approach: let cross-examination finish and produce a stable list first; trace cascades against that finished list, as its own resumable pass.
...
**Conflict-filtered inventory**
Building the transfer or change inventory from the list of items that surfaced as needing a decision during analysis, rather than from an exhaustive partition of the material itself. Content that requires no decision generates no entry in a decision-oriented list and is silently dropped — not because it was judged unnecessary, but because the process never asked about it. The best-behaved material — the kind with no conflict, nothing to argue about — is exactly what disappears this way.
→ Correct approach: partition the material into a complete inventory first, every unit assigned a status (transfer as-is / transfer with a resolved decision / deliberately excluded, stated why); apply the list of open decisions as a filter on top of that inventory afterward, never as a substitute for building it.

**Unprompted substitution**
Replacing material that will not resolve in the current environment — a broken reference, a missing dependency, an inaccessible path — with a working alternative the material's owner never asked for, in order to make something render or run. The mirror image of conflict-filtered inventory: that one drops material nobody asked to have dropped, this one replaces material nobody asked to have replaced. Both fail quietly, and both are typically discovered by whoever asked for the work, not by whoever did it.
→ Correct approach: carry an unresolvable reference forward exactly as written and say so explicitly, rather than silently substituting something that happens to work.
