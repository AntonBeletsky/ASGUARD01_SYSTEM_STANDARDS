...
| 1.3 Judgment | Not a command. Deciding whether a candidate `domino-next` returns is a real cascade stays exactly as unmechanized as dependency discovery always was — the search narrows what needs a look, it doesn't replace looking. |
...
| `add-descriptor <batch_id> --op OP --file PATH [--start ID --end ID \| --after ID_or_START] [--dest ID] [--payload-file F \| --payload-text T] [--depends-on IDS] [--category CODE] [--severity LEVEL] [--provenance direct\|domino] [--domino-hint P] [--domino-of ID]` | Phase 1/2: record one issue as one atomic edit. |
