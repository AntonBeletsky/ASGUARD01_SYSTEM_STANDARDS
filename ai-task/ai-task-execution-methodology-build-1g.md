...
## A Compositional Model of Build Dependencies — Six Dimensions, Two Qualifiers, a Durable Cache, and One Unit Per Turn

*Version 1g. Following `1f` in sequence, per `version-control-1b.md`'s own convention — prefix `ai-task-execution-methodology-build-`, version token `1g`.*
...
## What Changed from v1f

v1f's Cache-and-Commit Protocol removes hallucination risk from content transfer — on one condition it doesn't state out loud: that a script, external to the conversation, does the actual reading and writing. That condition holds for `EXTRACTED` content (a byte range that already exists in some source file) and fails for two real cases v1f didn't distinguish.

**Case one: the content has no source to extract from.** Two of `customer-account-1f.html`'s fourteen tabs (`tab-subscriptions`, `tab-item`) had no corresponding CSS/JS asset files anywhere in the package — unlike the other twelve. The likeliest reading of Case Study D, in hindsight: those two units needed to be *authored* — composed from a spec or a design reference, not copied from an existing fragment — and a cache populated by a script can't help with content the script has nothing to read.

**Case two: there is no script spanning the task at all.** A cache populated "by script, never by hand" (v1f's own rule) presumes a persistent execution environment sitting under the conversation for the whole task. Working through a plain chat session, one message at a time, that environment doesn't exist between turns — the only channel *any* content moves through, extracted or authored, is the model's own output in that turn. In that setting, "extraction" and "authoring" carry the same risk, because both ultimately mean the model producing the content as text.

**Both cases share one root cause v1f didn't name:** asking a model to produce several large, independent, content-bearing units within a single turn reproduces the exact failure Case Study D exhibits, regardless of how well the destination is marked or how rigorously a cache would validate the result afterward — because the gap opens at *authorship*, a step no downstream JSON gate can retroactively repair. A hash correctly confirms that what's in the cache matches what's in the cache; it says nothing about whether the model quietly wrote less than it should have in the first place.

**The fix:** stop treating "the file" as the atomic unit and start treating **the conversational turn** as the atomic unit. Exactly one unit — its full markup, its full CSS, its full JS, wired and self-consistent — gets authored, validated, and committed per turn. The destination file, complete up through whatever's been committed so far, is written out and delivered as an actual file at the end of every single turn — never merely described. The turn ends on a fixed, mandatory checkpoint message, and nothing about the next unit begins until that checkpoint's exact answer arrives. See *The One-Unit-Per-Turn Protocol*, below.

This version adds two classifications v1f didn't need: **content origin** (`EXTRACTED` vs. `AUTHORED` — does a verbatim source exist, or must this be composed) and **operating mode** (`SCRIPTED` vs. `TURN-GATED` — does an execution environment actually span Phase 2.6 through Phase 4, or does every unit's content pass through the conversation). v1f's batch pipeline is exactly right, unchanged, for `EXTRACTED` content in `SCRIPTED` mode. Everything else — `AUTHORED` content in either mode, or `EXTRACTED` content when no script spans the task — now runs through the One-Unit-Per-Turn Protocol instead.
...
- **Operating Mode — new in v1g** — `SCRIPTED` if a persistent execution environment genuinely spans Phase 2.6 through Phase 4 (so mechanical extraction and commit can run unattended, batched, exactly as v1f describes), or `TURN-GATED` if it doesn't — a plain chat session, one message at a time, is `TURN-GATED` by definition. State this explicitly; do not infer it partway through Phase 4 from how the work happens to be going.
...
AI:   [reads file names + sizes, does not open content yet]
      → "5 files loaded. Direction: MERGE (4 fragments → 1 shell). Roles: shell =
         ...
         orders,wishlist}, shared = page.css/page.js. Operating Mode: TURN-GATED
         (chat session, no execution environment spans the task). Confirming
         criterion before analysis."
```

**Do not assume a fragment's destination from its filename.** Confirm the mapping explicitly (see 1.1).

**Do not assume Operating Mode either.** `TURN-GATED` is the safe default any time this is uncertain — the cost of treating `SCRIPTED` work as `TURN-GATED` is some lost throughput; the cost of the reverse is Case Study D.
...
#### 1.4 Classify Content Origin — new in v1g

For every operation that will move or create content, decide which of two origins it has, before Phase 2 writes it into the Build Map:

- **`EXTRACTED`** — a verbatim source already exists (a fragment file, a marked range within one). Nothing needs to be composed; it only needs to be copied.
- **`AUTHORED`** — no verbatim source exists. The content has to be composed from a spec, a design reference, or a description of what's needed — by the model, in a turn, as text.

A concrete tell, illustrated by Case Study D: a tab with its own dedicated CSS/JS asset file already extracted alongside it is almost certainly `EXTRACTED` — the existence of a matching asset is itself evidence a source fragment existed. A tab with **no** matching asset anywhere in the materials is a strong candidate for `AUTHORED` — there may never have been a fragment to extract from in the first place. Treating an `AUTHORED` unit as if it were `EXTRACTED` — assuming a source exists somewhere and will turn up — is exactly how a unit ends up as a bare `TODO` with nothing behind it and no error raised anywhere.

**Rule:** every operation's Build Map entry carries a `content_origin` value; there is no third option and no "unsure" — if it's genuinely unclear, treat it as `AUTHORED` until a real source is located, since that's the assumption that fails safe (see *The One-Unit-Per-Turn Protocol*, below).
...
| `content_origin` — new in v1g | `EXTRACTED` / `AUTHORED` — set in 1.4; determines whether this operation can join a batch (Cache-and-Commit) or must run through *The One-Unit-Per-Turn Protocol* |
...
**Backpressure / flow control — new in v1g.** A queueing system that accepts work faster than it can reliably process it doesn't fail loudly — it silently degrades: items get dropped, truncated, or processed incompletely while the system reports throughput as if nothing were wrong. The fix in a queueing system is backpressure: the consumer processes one item, confirms it, and only then signals the producer for the next — never letting the producer get more than one item ahead of what's been durably confirmed. Case Study D is this failure mode wearing a different hat: fourteen units offered to the model in a scope that implicitly invited handling them all at once, no confirmation between them, no signal that any single one had actually landed before the next was attempted. *The One-Unit-Per-Turn Protocol*, below, is backpressure applied to a conversation.
...
**Standing rule, extended in v1g.** In `TURN-GATED` mode, the conversational turn boundary *is* the commit boundary — nothing about a unit is real until the turn that authored it has ended and its output has been delivered as an actual file. A unit described partway through a turn, before that turn's file is delivered, is exactly as provisional as a cache entry with a stale hash: it may already be sitting in the model's draft output, but nothing downstream — including the model's own next action — is entitled to treat it as done.

---

## The One-Unit-Per-Turn Protocol

*The Cache-and-Commit Protocol makes content transfer safe when a script does the transferring. This section makes it safe when nothing but the conversation does — which is the more common case, and the one Case Study D actually ran under.*

### When this applies

Exactly two conditions, either one sufficient on its own:

- `content_origin = AUTHORED` for the operation — there's no verbatim source, so the model must compose the content as output, regardless of what else is true about the environment.
- `Operating Mode = TURN-GATED` — no execution environment spans Phase 2.6 through Phase 4, so even `EXTRACTED` content ends up passing through the model's own output to get anywhere.

`EXTRACTED` content in genuine `SCRIPTED` mode is unaffected — v1f's batch pipeline runs exactly as written. Everything else runs through this section instead. When in doubt about which applies, this document's answer hasn't changed since Phase 0: treat it as the case that fails safe, which is this one.

### The rule

**Exactly one unit, fully finished, per conversational turn.** For the tab-merge scenario this document keeps returning to: one full tab — its markup, its CSS, its JS, correctly wired to each other and to the shell — gets authored and inserted in one turn. Not two tabs because the first was short. Not "the markup now, the styles next turn" for the same tab — a partially-wired unit is not a smaller version of the unit, it's a different, incomplete thing wearing the same name. The atomic boundary is the whole unit, and the whole unit fits in one turn or Phase 1 sized it wrong.

**Every turn ends by delivering the current destination file — not describing it.** Whatever has been committed so far, including everything from every prior turn, gets written to the actual output file and handed to the person as a file they can open, at the end of every single turn. A chat message saying "tab 3 has been added" with no updated file attached does not satisfy this — see the anti-pattern below. This is *Durability over volatile context* (above), applied at turn granularity: a unit isn't durable because the model produced correct-looking text about it; it's durable once it's sitting in a delivered file.

**Every turn ends on a fixed checkpoint message, and nothing proceeds without its exact answer.** The message must state, at minimum: which unit just finished, its position in the sequence (`N of M`, `M` fixed back in Phase 3), confirmation that all of its parts are present and internally consistent, confirmation the file has been delivered, and the exact token that authorizes the next unit. A reference implementation, in the project's own working language:

```
Вставил вкладку 3 из 14 (Orders) — HTML, CSS и JS подключены и
провалидированы, файл обновлён и прикреплён.
Чтобы вставить следующую вкладку, введите: продолжить
```

The English equivalent of the same fixed shape:
```
Inserted tab 3 of 14 (Orders) — HTML, CSS, and JS included and
validated; the file has been updated and attached.
To insert the next tab, reply: continue
```

**The accepted continuation token is a small, closed, explicit set — decided once, not inferred per reply.** `{"continue", "продолжить"}` is a reasonable pair for a bilingual project; add synonyms deliberately (`"next"`, `"go"`) if wanted, but the set is fixed in advance, not "anything that sounds like agreement." A reply outside that set is not a proceed signal, no matter how affirmative it reads.

### Handling a reply that isn't the token

A question about the tab just inserted, unrelated feedback, silence, or an ambiguous "ok great" are not authorization to continue. Answer what was actually asked, or wait — do not advance to the next unit until an accepted token arrives on its own. Advancing on a near-miss reply is the same mistake as accepting a near-miss hash: it treats "probably fine" as "confirmed."

### The ledger is still the cache — it's just filled one entry per turn instead of all at once

`AUTHORED` units still get a cache entry: `content` (what was actually authored, captured verbatim into the record — not summarized), `sha256_at_extraction` (hashed at the moment of authoring, standing in for "at the moment of extraction"), `status`, and, new here, `authored_at_turn`. Phase 2.7's completeness check (`planned == committed`) still runs — it's just read as a running count across many turns (`3 of 14 committed`) rather than a single before/after comparison. This is the same Level 0 check from Phase 5, now visible after every turn instead of only at the end, which is strictly more chances to notice a gap, not fewer.

### A discipline this protocol does *not* replace

Everything a script would have checked, the model checks by hand instead, every turn: does this unit's markup tag-balance on its own; does it actually include a CSS block and a JS block if the unit's definition requires both; do any ids or classes it introduces collide with a unit already committed in an earlier turn. None of this is optional just because there's no script running it — it's exactly the check *The Cache-and-Commit Protocol* runs mechanically, done manually, once per turn, because manual is what a `TURN-GATED` environment has.

### If a single unit doesn't fit in one turn

That's a Phase 1 sizing defect, not a reason to spill one unit across two turns while calling it one. Go back and split that specific unit under a stricter boundary (an unusually large tab might genuinely decompose into two Build Map operations instead of one) rather than quietly relaxing the one-per-turn rule for it.
...
**Fork, new in v1g — check `content_origin` and Operating Mode before anything else.** Rules 2–4 below describe the `SCRIPTED` + `EXTRACTED` path: a script reads the validated cache and splices mechanically, batched, unattended. For `AUTHORED` content, or for `EXTRACTED` content running in `TURN-GATED` mode, skip to *The One-Unit-Per-Turn Protocol* instead — rules 1 and 5–9 below still apply there too, just one unit and one turn at a time rather than batched.
...
**In `TURN-GATED` mode (new in v1g), Level 0 doesn't wait for the end.** It's the running count stated in every checkpoint message (`N of M`) — visible after every single turn, not computed once at the finish. Levels 1–3 still run as a full pass once, after the final unit's turn, over the complete destination; the per-turn check inside *The One-Unit-Per-Turn Protocol* (tag-balance, all parts present, no collision with earlier units) is a lighter version of Levels 1 and 3, run per-unit as a first line of defense, not a replacement for running the full Levels 1–3 once at the end.
...
Phase 0:    confirm shell + fragment list + shared/scoped resources + destination
            + Operating Mode (SCRIPTED / TURN-GATED — v1g)
Phase 1:    role-classify; scan each fragment for its payload boundary (check for
            ...
            classify each fragment's content_origin, EXTRACTED or AUTHORED (1.4 — v1g)
Phase 2:    Build Map — EXTRACT_INSERT per fragment payload, DEDUPE per NAME/
            ...
            every operation tagged with its content_origin (v1g)
...
Phase 2.6:  EXTRACTED + SCRIPTED — populate the cache in one batch, script-extracted,
            hash-stamped. AUTHORED, or TURN-GATED — see Phase 4 below instead (v1g)
Phase 2.7:  validation gate — planned-op count == cache-entry count, every hash
            ...
            (skipped per-unit in TURN-GATED mode — see Phase 4's ledger instead)
Phase 3:    plan the full destination-ordered sequence once, up front, regardless of
            mode — collisions/renames → shared-resource reconciliation → insertion
            order; this phase doesn't move content, so it is never turn-gated
Phase 4:    EXTRACTED + SCRIPTED — commit from the validated cache, batched, one
            destination at a time. Everything else — exactly one fragment, fully
            authored/committed, delivered as a file, per turn (One-Unit-Per-Turn
            Protocol — v1g); checkpoint message before each next fragment
Phase 5:    completeness count first (running, per turn, in TURN-GATED mode), then
            hash-match, dependency resolution by enforcement type, tag-balance —
            full pass once after the last fragment's turn
```
...
□ Was Operating Mode (SCRIPTED / TURN-GATED) stated explicitly in Phase 0, not assumed partway through Phase 4?
□ Was every operation's content_origin (EXTRACTED / AUTHORED) classified before Phase 2, with "unsure" treated as AUTHORED?
□ In TURN-GATED mode, was exactly one unit — fully wired, all its parts — completed per turn, never more?
□ Did every turn end with the current destination file actually delivered, not just described in the reply?
□ Did every turn end on the fixed checkpoint message, with the exact continuation token stated?
□ Was a reply outside the accepted-token set correctly treated as not-a-continue, rather than advancing on a near-miss?
```
...
### ❌ Authoring More Than One Unit Per Turn "to Save Time"
```
Bad:  the current tab was short, so write two or three more while
      already in there
Good: one unit per turn, always — a short unit is an argument for
      speed, never an argument for batching
```
Reason: the size of the unit just finished says nothing about the risk of the *next* one — batching because the easy ones are cheap is how the two hardest units in a set of fourteen end up being the two that get shortchanged, exactly as in Case Study D.

### ❌ Describing an Insertion Without Delivering the File
```
Bad:  "Tab 3 (Orders) has been added with its CSS and JS" — no file
      attached, or an attached file that's stale
Good: the actual, current, cumulative destination file is written out
      and delivered at the end of every turn, no exceptions
```
Reason: a description is a claim; a delivered file is a fact the next turn — or a different session entirely — can check against. *Standing rule, extended in v1g* exists precisely so nothing downstream has to take the model's word for it.

### ❌ Treating Any Reply as Permission to Continue
```
Bad:  the person replies "looks good" or asks a small question about
      the tab just added — advance to the next unit anyway
Good: only the fixed, agreed continuation token advances the sequence;
      anything else gets answered or waited on
```
Reason: "looks good" is feedback, not the word that was actually specified. Accepting near-misses here reintroduces exactly the ambiguity a fixed checkpoint token exists to remove.

### ❌ Skipping the Checkpoint Because the Remaining Units Look Routine
```
Bad:  units 4 through 14 are all "the same shape" as unit 3, so do them
      in one final pass without individual checkpoints
Good: every unit gets its own turn and its own checkpoint, including
      the ones that look like copies of a pattern already proven out
```
Reason: "the same shape" was also true of twelve of Case Study D's fourteen tabs — and it was the two that weren't quite the same shape (no matching asset files) that got silently dropped. Routine-looking units are exactly where attention relaxes first.
...
### Case Study E — The Same Two Tabs, Turn-Gated

Picking up exactly where Case Study D left off: `tab-subscriptions` and `tab-item`, both `content_origin: AUTHORED` (1.4 — neither had a matching CSS/JS asset anywhere in the package), both handled this time under `Operating Mode: TURN-GATED`, one turn each.

```
Turn 12 (of 14):
  AI:   [authors tab-subscriptions in full: markup + scoped CSS + JS,
         wired to the existing shell nav; checks its own tag balance and
         confirms no id collision with the 11 tabs already committed]
        → delivers customer-account-1f.html, now 12 of 14 tabs complete
        → "Вставил вкладку 12 из 14 (Subscriptions) — HTML, CSS и JS
           подключены и провалидированы, файл обновлён и прикреплён.
           Чтобы вставить следующую вкладку, введите: продолжить"
  User: "продолжить"

Turn 13 (of 14):
  AI:   [authors tab-item in full, same discipline, checks against all
         12 tabs committed so far, not just the one before it]
        → delivers customer-account-1f.html, now 14 of 14 tabs complete
        → "Вставил вкладку 14 из 14 (Item) — HTML, CSS и JS подключены
           и провалидированы, файл обновлён и прикреплён. Это была
           последняя вкладка — карта сборки полностью закрыта (14 из 14)."
```

Two things this trace makes visible that Case Study D's single-pass version couldn't: first, the running count (`12 of 14`, `14 of 14`) was stated to the person after every turn, not silently tracked nowhere until someone opened the file — a gap of the kind Case Study D had would show up as a stalled count, not a surprise. Second, each turn delivered a real, complete-so-far file — so even if the session had ended after Turn 12, the twelve genuinely finished tabs would already be sitting in a downloadable file, and the fact that two remained would be stated, not implied.
...
7.  (TURN-GATED mode, new in v1g) Did any single turn author more than
    one unit? Did every turn end with a delivered file and the fixed
    checkpoint message? Was every continuation an exact accepted token,
...
- Units per turn in TURN-GATED mode (target: exactly 1, always) and files delivered vs. turns completed (target: equal — a turn with no delivered file is a turn that didn't finish)
- Continuation replies received vs. continuation replies matching the accepted-token set exactly (a gap here means near-miss replies are being treated as approval somewhere)

---

*This methodology is a companion to `ai-task-execution-methodology.md`. Version 1b replaced v1a's eleven-item enumerative taxonomy with a compositional model. Version 1c added a Marking Pass. Version 1d repositioned it to Phase 2.5 and generalized its marker to a `sources=[...]` list. Version 1e was the first revision informed by an actual Phase 0–5 run rather than reasoning about the model in the abstract. Version 1f is informed by a second real run — a 14-tab merge, `customer-account-1f.html`, in which every dependency check this document already specified passed cleanly, and the defect that shipped anyway (two of fourteen tab-panes left as bare TODO comments) was one this document had no formal check for: not "is this correct" but "did this happen at all." The Cache-and-Commit Protocol, and the Reliability Model that motivates it — write-ahead logging, two-phase commit, hash verification at every hand-off, durability over conversation context — exist to make that count a mandatory, mechanical gate rather than something a person has to remember to run. Version 1g goes one level further: the Cache-and-Commit Protocol presumes a script does the transferring, and two real situations mean it sometimes doesn't — content with no verbatim source to extract (`AUTHORED`, not `EXTRACTED`), and sessions with no execution environment spanning the task at all (`TURN-GATED`, not `SCRIPTED`). For both, this version makes the conversational turn itself the atomic, durable, checkpointed unit: one unit authored, validated, and committed per turn; the current file delivered, never merely described, at the end of every turn; and a fixed checkpoint message whose exact answer — not any agreeable-sounding reply — is the only thing that authorizes the next unit. Case Study E re-runs Case Study D's own two dropped tabs under this discipline and shows the running count stated aloud, turn by turn, rather than silently going unchecked until someone happened to open the file. The three original illustrative sources remain as Case Studies A–C.*
