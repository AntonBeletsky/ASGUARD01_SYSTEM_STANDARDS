...
*Version 1c. Following `1b` in sequence, per `version-control-1b.md`'s own convention — prefix `ai-task-execution-methodology-build-`, version token `1c`.*
...
v1b fixed *what* gets checked — six dimensions instead of eleven ad hoc types. It didn't address *when* the checking turns into something physically verifiable. The Build Map (Phase 2) and Assembly Plan (Phase 3) are both external artifacts — JSON and a checklist that live in the conversation, not on the units themselves. Nothing stops the gap between "I planned this" and "I'm now re-deriving it from a re-scan, or from memory" from opening up silently the moment Phase 4 starts.

v1c adds **Phase 3.5 — Marking Pass**: a cheap, reversible edit that plants a comment marker at every source boundary and every destination anchor *before* any real content moves, verified by a single grep before Phase 4 is allowed to start. This directly answers a concrete question that prompted it: does the plan fix entry points and insertion sections *before* the token-costly work starts, physically, not just on paper? Before v1c: no. The map recorded coordinates; nothing forced them onto the actual units where they'd survive a compacted context and stay checkable independently of the conversation that produced them.
...
Knowing what to check and knowing where to cut both still have to survive the trip from "I've figured it out" to "I've done it." That's what Phase 3.5 is for.
...
| `source.lines` | Exact range — what Phase 3.5 turns into a physical `BUILD:EXTRACT` marker pair |
| `destination.anchor` | The literal marker or exact insertion point — what Phase 3.5 turns into a `BUILD:INSERT` marker |
...
This plan is still paper — a grouped, ordered checklist, not something the units themselves know anything about. **Phase 3.5 is where that stops being true.**

### Phase 3.5 — Marking Pass

The Build Map (Phase 2) and the Assembly Plan (Phase 3) are both external to the actual units — JSON and a checklist that live in the conversation, not on disk. This phase turns them into something physically checkable: a lightweight, reversible edit that plants a marker at every source boundary and every destination anchor, *before* a single byte of real content moves.
...
At every DESTINATION anchor:
```html
<!-- in customer-account-1e-clean.html, replacing the TODO at line 523 -->
<!-- BUILD:INSERT op=build-001  source=account-messages-1b.html#build-001
...
**Verification, before Phase 4 starts:**
```bash
grep -rn "BUILD:EXTRACT START\|BUILD:EXTRACT END\|BUILD:INSERT" .
```
For every `op=build-NNN`: exactly one EXTRACT START, exactly one matching EXTRACT END, and exactly one INSERT referencing it elsewhere. Anything less is a Phase 3.5 defect — cheaper to fix now, before Phase 4 has moved content around it, than after.

**What this buys Phase 4:** extraction stops being "re-find the boundary, then copy" and becomes "find the marker, take everything between START and END, go to the matching INSERT, replace it." The expensive work now runs against fixed, physical, grep-verified coordinates instead of re-derived ones.

**Cleanup.** Markers are scaffolding. Strip them as part of Phase 5, or immediately after, unless the project explicitly wants to keep them as an audit trail of how the assembly was built — state which; don't leave it ambiguous.
...
**2. Find the marker; don't re-derive the range.** After Phase 3.5, extraction is a lookup, not a re-scan or a memory exercise:
```python
def extract_marked_block(text, op_id):
    start_tag = f"<!-- BUILD:EXTRACT START op={op_id} -->"
    end_tag = f"<!-- BUILD:EXTRACT END op={op_id} -->"
    start = text.index(start_tag) + len(start_tag)
    end = text.index(end_tag)
    return text[start:end]
```
If the Build Map's line numbers and the planted markers disagree, that's a signal to re-verify before proceeding — not to trust whichever is more convenient.

**3. A missing marker is a Phase 3.5 defect, not a Phase 4 workaround.** Go back and plant it — re-verifying the boundary with *Precise Pattern Matching* — rather than guessing a range on the spot.
...
**Marker Cleanup Check.** Confirm every `BUILD:` marker planted in Phase 3.5 has been removed from the final output, or that keeping them was a stated decision, not an oversight:
```bash
grep -rn "BUILD:EXTRACT\|BUILD:INSERT\|BUILD:NOTE" . && echo "residual markers — clean up before delivery"
```
A residual marker in shipped output is a Phase 5 defect exactly like a dangling reference is.
...
Run against `account-messages-1b.html`, this finds the `<section class="messages-container">` opened at line 289 closing at line 576 — confirmed independently by the fragment's own closing comment, `<!-- /messages-container -->`. This is exactly the boundary Phase 3.5 plants its `BUILD:EXTRACT` markers around. The same technique generalizes to JS/JSON objects (`{ }`) and code (`( )`). Beyond a single well-known bracket type, use an actual parser.
...
11. Marker convention for this task's file types — the comment syntax to use
    in Phase 3.5, and confirmation that syntax is legal in every format involved
```
...
| Plan exists only in conversation, not on the units | Phase 3.5 plants it as grep-verifiable markers before Phase 4 |
...
Phase 3.5: plant BUILD:EXTRACT/INSERT markers at every boundary and anchor;
           verify every op id pairs up with one grep before Phase 4 starts
...
Phase 3.5: plant BUILD:EXTRACT markers at each verified cut point — both the
           piece staying behind and the piece leaving — and verify pairing
...
Phase 3.5: mark the canonical source and every consumer's relink point;
           verify before touching any consumer
...
Phase 3.5: mark both the old and new boundaries together, since source and
           destination can be mid-transition simultaneously here
...
□ Planted BUILD:EXTRACT/INSERT markers at every boundary and anchor BEFORE
  starting the expensive copy work, and verified every op id pairs up?
...
**What the Marking Pass looks like for this case** (Phase 3.5, before any content moves):
```html
<!-- in customer-account-1e-clean.html, replacing the TODO at line 523 -->
<!-- BUILD:INSERT op=build-001  source=account-messages-1b.html#build-001
     ALSO: op=build-002 (script) -> new file customer-account-messages.js
...
<!-- BUILD:EXTRACT END op=build-001 -->
```
One command — `grep -rn "BUILD:" *.html` — confirms `build-001` has exactly one START, one END, and one INSERT before Phase 4 touches a single byte of the actual payload.
...
grep -rn "BUILD:EXTRACT\|BUILD:INSERT" .            # after 3.5: confirm every op id pairs up
```
...
10. Were markers planted and pairing-verified in Phase 3.5 BEFORE Phase 4
    started, and cleaned up (or intentionally kept) by Phase 5?
```

**Numerical indicators to record before/after:** unit counts; total lines/bytes (should reconcile); reference-registry entries, resolved vs. dangling, by dimension; Cardinality conflicts found/resolved; Enforcement mix (how many CHECKED vs. CONVENTION_ONLY findings, since the latter carry more risk per finding); marker count planted vs. cleaned up.

---

*This methodology is a companion to `ai-task-execution-methodology.md`. Version 1b replaced v1a's eleven-item enumerative taxonomy with a compositional model — six dimensions, two cross-cutting qualifiers, one standing rule. Version 1c adds Phase 3.5 — Marking Pass, after confirming that neither prior version forced the Build Map's coordinates onto the actual units before Phase 4 started: entry points and insertion sections were recorded in an external, conversation-bound map, never planted as something grep-verifiable on the units themselves. The three illustrative sources (an HTML/CSS/JS page assembly, a widget/orchestrator component system per `containerization-11.md` and `containerization-orchestration-2.md`, and a version-token parsing problem per `version-control-1b.md`) remain as Case Studies and feed the Catalog of Observed Patterns — which opens, deliberately, by stating that it is not exhaustive and is not meant to be.*
