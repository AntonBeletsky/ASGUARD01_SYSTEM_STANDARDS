...
*Version 1e. Following `1d` in sequence, per `version-control-1b.md`'s own convention — prefix `ai-task-execution-methodology-build-`, version token `1e`.*
...
**What this buys everything downstream:** Assembly Plan groups operations whose entry/exit points are already physically confirmed, not raw JSON. Execution stops being "re-find the boundary, then copy" and becomes "find the marker, take everything between START and END, go to the matching INSERT, replace it." The expensive work now runs against fixed, physical, grep-verified coordinates instead of re-derived ones.
...
By this point, every operation's entry and exit points are already physically marked and grep-verified (Phase 2.5) — this phase is purely about execution **order**, not discovery. The map becomes a grouped, ordered tracker regrouped around **destinations**.
...
Nothing here changes what Phase 2.5 already marked — only the sequence Phase 4 works through it.
...
**2. Find the marker; don't re-derive the range.** By this point extraction is a lookup, not a re-scan or a memory exercise:
```python
...
```
For a destination whose `sources` list has more than one entry, extract each by its own `op_id` and concatenate in list order — the list itself already fixed that order back in Phase 2.5. If the Build Map's line numbers and the planted markers disagree, that's a signal to re-verify before proceeding — not to trust whichever is more convenient.

**3. A missing marker is a Phase 2.5 defect, not a Phase 4 workaround.** Go back and plant it — re-verifying the boundary with *Precise Pattern Matching* — rather than guessing a range on the spot.
...
**Marker Cleanup Check.** Confirm every `BUILD:` marker planted in Phase 2.5 has been removed from the final output, or that keeping them was a stated decision, not an oversight:
```bash
...
Phase 2.5: plant BUILD:EXTRACT/INSERT markers at every boundary and anchor found
           in 1.3/Phase 2 — including ALSO: notes and sources=[...] for any
           destination fed by more than one operation; verify pairing with one grep
Phase 3:   plan by destination — collisions/renames → shared-resource
           reconciliation → insertion in logical order
Phase 4:   cut by exact range (found via marker, not re-derived), resolve
           collisions atomically, validate per fragment
...
Phase 2.5: plant BUILD:EXTRACT markers at each verified cut point — both the
...
Phase 2.5: mark the canonical source and every consumer's relink point;
...
Phase 2.5: mark both the old and new boundaries together, since source and
...
10. Were markers planted and pairing-verified in Phase 2.5 — right after the
    Build Map, before Assembly Plan — and cleaned up (or intentionally kept)
    by Phase 5?
11. Does every `sources=[...]` list contain every op id it should, with no id
    appearing in more than one list?
```
