...
A specification, a real model written in it, and the tooling used to inspect and edit that model — six files, meant to sit together in one folder.
...
| 5 | `diagrameditor.html` | HTML | Editable diagram — drag entities to reposition, draw relations between them, edit fields via dropdowns, undo/redo, export/import JSON |
| 6 | `readme.md` | Markdown | This file |
...
All 6 files must stay in the same folder — `EMJLDesign.html` links to the others with plain relative paths (`<a href="university_erd.html">` etc.), and those links only resolve if the files are co-located. Files 3, 4, and 5 load Bootstrap from `cdn.jsdelivr.net` at runtime (file 3 also loads Mermaid), so an internet connection is needed to see them fully styled/rendered — opening them offline still shows the content, just unstyled.

`diagrameditor.html` has no live save: edits exist only in the browser tab's memory. Use its Export button to download a JSON snapshot, and Import to reload one later.
