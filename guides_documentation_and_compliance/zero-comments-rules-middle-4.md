...
- Plain-Language Notes — Scope · The Five Fields · Language Register · Format and Marker · Beyond HTML, CSS, and JS
- JavaScript — Class Header · Public Methods · Plain-Language Notes (JS) · The Vanishing Method · The Undying Method · Section Headers · Inline "Why" Comments · The Short-Lived Field · The Long Stretch · Component Entry Point · The Templated Comment · What Gets No Comment
- CSS — File Header · Two-Tier Section Hierarchy · Component Block Description · Plain-Language Notes (CSS) · The Trailing Description · Inline Property Notes · The Aside That Never Ends · The Double Comment · Responsive Blocks · The Media Query Swallow · Accessibility Block · What Gets No Comment
- HTML — Root Container · Plain-Language Notes (HTML) · The Vanishing Region · The Reappearing Element · Major Layout Regions · data-* Attributes · ARIA Attributes · The Silent Accommodation · Third-Party Embed Wrappers · What Gets No Comment
- Editing Existing Files Safely — The Merged Header · General Practice
- Section Header Format Reference
- Comment Density Summary
- Pre-Release Checklist — Remove · Verify · The Test · The Count
- Quick Reference
...
Is this an Architecture or Component-level unit (see §Plain-Language
Notes) that a non-technical reader needs to understand?
  └─ YES → Add a Plain-Language Note next to the comment above —
           this is additive, never a replacement for it
...
Every rule up to this point is written for a reader who can read code. This
section adds a second, narrower rule for a different reader: someone
maintaining the project who cannot. A store owner editing copy, a support
agent tracing a bug report, a buyer who has never opened a `.js` file — the
"junior developer" from *The Test* later in this guide can already read a
JSDoc block and a Tier A header. This reader can't, and needs a note written
for them specifically, not a simplified version of the comment sitting next
to it.

This is an additive rule. Nothing above changes: JSDoc stays technical,
inline "why" comments keep using precise industry terms, Tier A/B headers
keep their existing format. A Plain-Language Note sits *next to* the
developer-facing comment for a unit, never instead of it, and only on the
units defined below — not on every class, method, or rule that already gets
one of the comments described earlier in this guide.

### Scope — Which Units Get One

Two levels, both borrowed directly from categories this guide already
defines. No new judgment call, no separate scoring system:

| Level | What qualifies | Same unit as |
|---|---|---|
| Architecture | A whole-file or whole-page structural landmark | CSS Tier A structural sections (`LAYOUT`, `THEME TOKENS`, `RESET & BASE`), the HTML root container, the JS class itself |
| Component | A named, self-contained, reusable unit with its own boundary | A CSS Tier A `COMPONENT: *` section, an HTML component root, a JS public method that represents one distinct capability |

This narrows, it never widens: not every Tier A section is "architecture" in
the sense meant here — a single `RESPONSIVE: MOBILE` breakpoint tweak
usually isn't — and not every public method is a "component" — a one-line
formatter isn't. Reserve this for units where a non-technical reader
genuinely needs to know the unit exists, what it's for, and what not to
touch. If a unit doesn't already qualify for a comment under the rules
earlier in this guide, it doesn't qualify here either.

Applying this to every unit that technically qualifies is still the wrong
instinct in a large file — see the density diagram at the very top of this
guide. This rule adds a second block on top of an already-commented file;
"does a non-technical reader actually need this one explained" is a real
filter, not a formality.

### The Five Fields

Every Plain-Language Note answers five questions, in this order, one
sentence each:

| Field | Question | Vocabulary |
|---|---|---|
| `Type:` | What kind of thing is this? | `container`, `component`, `part`, `function`, or `element` |
| `Does:` | What does it actually do, in plain terms? | free text, no jargon |
| `Connected to:` | What else does it depend on, or affect? | names of other components, files, classes, tokens |
| `Why:` | Why was it built this way? | free text, plain-language reasoning |
| `Watch for:` | What should someone changing this be careful of? | concrete and actionable, not generic caution |

A qualifying unit gets all five fields — an incomplete note is worse than no
note, because it still looks authoritative. If a field genuinely has
nothing to report (a component with no dependencies), write that ("Nothing
else depends on this") instead of dropping the line; a missing field should
never be silently indistinguishable from "connected to nothing."

`Type` is the one field with a closed vocabulary, so pick the closest fit
rather than inventing a new word:

- **container** — wraps and orchestrates other elements (a page's root
  `<div>`, a JS controller class)
- **component** — a named, reusable, self-contained unit (a CSS
  `COMPONENT: *` section, a card, a modal)
- **part** — a sub-piece of a larger component that doesn't stand on its
  own (a card's header row, a form's validation block)
- **function** — a single method or capability, most often in JS (a public
  method, a calculation, an event handler)
- **element** — a single tag or rule too small to be its own component (an
  icon, a badge, a single design token)

### Language Register

Plain language, not simplified jargon. Don't write "uses event delegation" —
write "one listener on the parent handles clicks on every row, including
rows added later." If a technical term can't be avoided (CSS, API,
database), use it once and explain it in the same sentence, the first time
it appears in a file.

This is the one place in this guide where industry terms are the thing to
avoid, not the thing to require. The *Verify* item later in this guide about
using precise terminology (Memory Safety, Event Delegation, etc.) governs
the developer-facing comments described earlier in this guide — it does not
apply to Plain-Language Notes.

### Format and Marker

A Plain-Language Note is its own block, in the host language's native
comment delimiters, placed directly after the existing developer-facing
comment for that unit — JSDoc, Component Block Description, or Root
Container note — never merged into it. Every field opens with its label and
stays to one sentence — a sentence that wraps onto a second physical line is
normal, a field that needs three is not. The block opens with the same
fixed first line every time, so it can be found by searching a file for
that exact string, without numbering anything:

```
Plain-language note
```

A complete block is the marker plus five one-sentence fields — typically
six to eleven physical lines once wrapping is accounted for. If any field
is spilling past two wrapped lines, it's drifting into a tutorial — cut it
back to the one thing that actually matters.

Everything in *Absolute Rule: No Comment Ships Unclosed* and *Mandatory
Delimiter Balance Check*, earlier in this guide, applies here without
exception. A block this size is exactly the shape that produces *The
Trailing Description* and *The Aside That Never Ends* — see the worked
example in each language section below, and re-run the balance check on any
file where a Plain-Language Note was added or edited.

### Beyond HTML, CSS, and JS

The rule itself isn't tied to any one language: the five fields, the scope,
and the register above apply to any file type in the project. Only the
delimiter changes. For a language this guide doesn't cover directly —
SCSS/LESS, PHP, a templating language — use that language's own comment
syntax and hold it to the same closing discipline this guide requires
everywhere else.

One format has no answer: strict JSON has no comment syntax, and adding one
breaks the file. Don't invent a `"_comment"` key to work around this — put
the Plain-Language Note in an adjacent file (a sibling `README` or a `.md`
note next to the config) instead of inside the JSON itself.
...
### Plain-Language Notes (JS)

Placed directly after the JSDoc block it accompanies — never inside it.
A class gets one at the Architecture level; a public method that stands on
its own as a distinct capability gets one at the Component level.
...
/* Plain-language note
   Type: container
   Does: switches between the tabs on the account page and keeps the web
   address in sync, so a link to a specific tab opens that tab directly.
   Connected to: every button on the page with a data-target attribute.
   Why: a custom script replaces the page-builder's built-in tabs so a tab
   can be opened straight from a saved or shared link.
   Watch for: renaming a tab's id breaks any link that points to it. */
class AccountPageController {
```

Same failure shape as *The Vanishing Method* just below, only now it's the
new block that loses its closer instead of the JSDoc:

```js
/* ✗ WRONG — closer missing after "Watch for" */

/* Plain-language note
   Type: container
   Does: switches between the tabs on the account page.
   Connected to: every button with a data-target attribute.
   Why: supports opening a tab straight from a shared link.
   Watch for: renaming a tab's id breaks any link that points to it.
class AccountPageController {
```

`class AccountPageController { ... }` — the entire class — is now comment
text, exactly as in *The Vanishing Method* below, until some later `*/`
elsewhere in the file accidentally supplies a closer.
...
At the Component level, a public method gets the same five fields —
no ✗/✓ pair needed here, the closing risk was already shown above:

```js
/**
 * Switches to the tab matching the given id and updates the URL hash.
 *
 * @param {string} tabId - The id of the tab to activate.
 */
/* Plain-language note
   Type: function
   Does: opens a specific tab by its id and updates the address bar to match.
   Connected to: the same data-target buttons the class already listens to.
   Why: exposed as its own method so other code — or a future "back to
   overview" button — can jump to a tab without simulating a click.
   Watch for: passing an id that doesn't match any tab does nothing; it
   doesn't throw or show an error. */
openTab(tabId) {
```
...
### Plain-Language Notes (CSS)

Placed directly after the Component Block Description for that section —
never merged into it. A Tier A structural section (`THEME TOKENS`, `LAYOUT`)
gets one at the Architecture level; a Tier A `COMPONENT: *` section gets one
...
/* Plain-language note
   Type: component
   Does: shows the account page's side navigation menu.
   Connected to: the Responsive section below, which turns it into a
   horizontal scroll bar on phones and tablets.
   Why: kept sticky on desktop so the menu stays visible while scrolling.
   Watch for: changing its width here also changes how much room is left
   for the content area next to it. */

.sidebar { ... }
```

Same failure shape as *The Trailing Description* just below:
...
/* Plain-language note
   Type: component
   Does: shows the account page's side navigation menu.
   Connected to: the Responsive section below.
   Why: kept sticky on desktop so the menu stays visible while scrolling.
   Watch for: changing its width here also changes the content area next to it.

.sidebar { ... }
```

`.sidebar { ... }` and every rule after it are now comment text, until a
later `*/` elsewhere in the file happens to close it — the same mechanism
as the real Wallet incident described earlier in this guide.
...
At the Architecture level, the same five fields describe a structural
section instead of a component — no ✗/✓ pair needed, the closing risk was
already shown above:
...
/* Plain-language note
   Type: container
   Does: holds every color, spacing, and radius value the rest of the file
   uses, so the whole look can be changed from one place.
   Connected to: every color and spacing value in this file — none of them
   are written as fixed numbers.
   Why: lets a client re-theme the widget by editing a handful of values
   here instead of hunting through the whole file.
   Watch for: renaming a variable here breaks every rule that references
   it — search the file for the old name before renaming. */
...
### Plain-Language Notes (HTML)

Placed directly after the Root Container comment — never merged into it.
The page's root container gets one at the Architecture level; a named
component root inside it (the sidebar, a card, a form) gets one at the
Component level.
...
<!-- Plain-language note
     Type: container
     Does: holds the whole account page — the side menu and the main
     content area both live inside it.
     Connected to: AccountPageController (JS), which finds this element
     by its class name and controls everything inside it.
     Why: built as one script-driven container instead of separate
     page-builder sections, so the tabs can share one URL-based router.
     Watch for: this element's class name must stay exactly
     account-page-container, or the script will not find it. -->
<div class="account-page-container">
```

Same failure shape as *The Vanishing Region* just below — and the largest
blast radius of the three languages, since nothing further down the file
limits the damage:
...
<!-- Plain-language note
     Type: container
     Does: holds the whole account page.
     Connected to: AccountPageController (JS), which finds this element
     by its class name.
     Why: built as one script-driven container so the tabs can share
     one URL-based router.
     Watch for: this element's class name must stay exactly
...
Everything inside — the sidebar, the content area, and anything further
down the file — is comment content unless some unrelated later `-->`
happens to exist further down.
...
At the Component level, a named element inside the root gets its own note
instead of describing the whole page — no ✗/✓ pair needed, the closing
risk was already shown above:

```html
<!-- Sidebar navigation — component root.
     Switches to a horizontal scroll bar on mobile; see the CSS
     Responsive section. -->
<!-- Plain-language note
     Type: component
     Does: shows the account page's side navigation menu.
     Connected to: the .sidebar rules in the CSS file, which control how
     it looks and how it collapses on small screens.
     Why: kept as its own element so the CSS Responsive rules can target
     it without affecting the rest of the page.
     Watch for: this must stay a direct child of the page's root
     container, or the layout grid stops lining it up correctly. -->
<aside class="sidebar">
```
...
### Plain-Language Note (any language)
```
Plain-language note
Type: ...
Does: ...
...
Watch for: ...
```
Wrapped in the host language's own comment delimiters (`/* */` in CSS and
JS, `<!-- -->` in HTML) — see §Plain-Language Notes for the full rule and a
worked example per language.
...
| Architecture-level unit (any language) | Plain-Language Note, 5 fields | Always — see §Plain-Language Notes |
| Component-level unit (any language) | Plain-Language Note, 5 fields | Always — see §Plain-Language Notes |
...
- [ ] No comment block exceeds 3 lines (except class-level JSDoc and
      Plain-Language Notes)
...
- [ ] Every Architecture-level and Component-level unit has a Plain-Language
      Note with all five fields present
- [ ] Every Plain-Language Note uses plain language, not industry jargon
- [ ] Plain-Language Notes are balanced and closed under the same delimiter
      rules as every other comment in this guide
- [ ] Every Plain-Language Note still matches what the unit actually does —
      re-check it whenever the code next to it changes
...
Plain-Language Notes (Architecture/Component)
                                        Numbers of any kind
```
