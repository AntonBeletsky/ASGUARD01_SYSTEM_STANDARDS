...
|---|---|---|
| Architecture | A whole-file or whole-page structural landmark | CSS Tier A structural sections (`LAYOUT`, `THEME TOKENS`, `RESET & BASE`, each `RESPONSIVE: *`), the HTML root container, the JS class itself |
...
Container note — never merged into it. Every field is one line, opening
with its label. The block opens with the same fixed first line every time,
so it can be found by searching a file for that exact string, without
numbering anything:
...
A complete block is six lines (the marker plus the five fields) before
delimiters. If it's running longer, `Watch for` is drifting into a
tutorial — cut it back to the one thing that actually matters.
...
exception. A six-line block is exactly the shape that produces *The
...
Plain-Language Notes (landmarks)
                                        Numbers of any kind
```
