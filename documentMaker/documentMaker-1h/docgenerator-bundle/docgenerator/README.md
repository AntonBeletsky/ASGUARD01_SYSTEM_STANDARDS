...
- **Responsive down to a phone**, not just a smaller desktop layout: below
  880px the sidebar becomes a slide-out drawer (hamburger button, dimmed
  backdrop, tap outside or pick a file to close it), the topbar sheds its
  lowest-value elements (timestamp, file-count chips — the same counts are
  already in the Overview cards) rather than clipping them, and every
  table that could otherwise force the whole page to scroll sideways
  scrolls internally instead. A "Collapse all / Expand all" control sits
  above the file tree for projects with many top-level folders.
...
Scan-to-HTML is one shot by default — nothing here is required. But every
diagnostic in the output is a heuristic, and on an unusual project (heavily
dynamic DOM, a single-file-per-type layout where the dependency graph is
near-trivial) the default output can include sections that aren't useful
*for that specific codebase*. `--plan-only` writes a flat JSON of binary
(1/0) switches — pre-filled with real finding counts, not guesses — that
can be reviewed and edited before any HTML is written; `--from-plan` then
renders respecting it. Every default is 1 ("show it"): the plan is a lever
for a person or an agent to pull, never a second layer of heuristics
deciding things on its own. See `doc-generator-cli/README.md`, "Optional:
the generation plan," for the full schema and an example.
